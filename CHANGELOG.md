...
- The Settings tab's Tunnel card now has a "Save QR Code (SVG)" button that downloads the racer signup QR code as an SVG file. This saved code points at the plain racer page URL without any event-specific query parameters, so the same printed QR code works for every event. The QR codes shown on the admin Settings card and the projector are unchanged and still include the current event details.
- The Race Desk tab now has a "Racer Queue Signups" card at the top that closes the queue to new racer self-service signups — useful for draining the queue before a tournament. A toggle shows the open/closed status, and a "Closed message" field (with a "Save Message" button) sets what racers see; leaving it blank falls back to "The queue is currently closed." While closed, the racer page's header status reads your message (or "QUEUE CLOSED"), and the Join Queue, Solo Run, and Challenge controls are hidden behind that message — but the upcoming-races list and each racer's current queue position still show. Adding racers from the admin panel still works regardless. The queue is open by default.
...
- The horizontal race display themes (track, Oregon Trail '90, Frontier Trail) now scale as one piece: the lane cards, riders, track bars, gaps, and readouts all grow and shrink together with the size of the race box, so the composition looks the same from a 720p screen up to a 4K projector — only the overall size changes, with no sudden jumps at particular window sizes. The two lanes are always the same height and no longer stretch to fill the screen; leftover space now shows as balanced padding above and below the lanes instead of inflating the lane cards, and a solo race shows its single lane centered. The glow lab preview also now renders the lanes at a size that fits its preview box.
