...
- The Race Desk tab now has a "Racer Queue Signups" card at the top that closes the queue to new racer self-service signups — useful for draining the queue before a tournament. A toggle shows the open/closed status, and a "Closed message" field (with a "Save Message" button) sets what racers see; leaving it blank falls back to "The queue is currently closed." While closed, the racer page hides the Join Queue, Solo Run, and Challenge controls behind that message but still shows the upcoming-races list and each racer's current queue position. Adding racers from the admin panel still works regardless. The queue is open by default.
