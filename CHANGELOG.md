...
- Added a Glow Lab (Settings → Lab Pages → Open Glow Lab) for designing the lane glow and lead-change flash. Position each racer in their lane, set each lane's glow and flash levels by hand, and tune each cue's size, blur, scale, opacity, and falloff live across every race graphic — with Copy CSS buttons to capture the tuned values. The layout leaves room for the upcoming companion cues (top-speed flare, speed streaks).
